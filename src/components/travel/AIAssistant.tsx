...
const AIAssistant = ({ weather, distance }: { weather: string; distance: number }) => {
  ...
  const [chatStatus, setChatStatus] = React.useState('Checking...');
  ...
  React.useEffect(() => {
    ...
    aiApi.status()
      .then((response) => {
        if (!active) return;
        const status = response.data?.data?.message || 'Chatbot ready';
        setChatStatus(status);
      })
      .catch(() => {
        if (!active) return;
        setChatStatus('Chatbot unavailable');
      });
  ...
  const handleSend = async (text: string = input) => {
    ...
    try {
      ...
      setMessages(prev => [
        ...
        {
          role: 'ai',
          content: "I couldn't reach the assistant service. Please make sure the backend is running, then try again."
        }
      ]);
    } finally {
      setIsTyping(false);
      ...
      <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">{chatStatus}</p>
          </div>
        </div>
