...
import { estimateTripMetrics } from '@/utils/tripEstimates';
...
const Explore = () => {
  ...
  const totalCost = Math.max(metrics.cost, 1400);
        ...
        addedPlaces.length > 0
          ? addedPlaces.map((p, i) => ({
              ...
              from:
                i === 0
                  ? 'Hyderabad'
                  : addedPlaces[i - 1].name,

              to: p.name,

              duration: Math.round(metrics.durationMinutes / Math.max(addedPlaces.length, 1) / 1.25),

              cost: Math.round(totalCost / Math.max(addedPlaces.length, 1)),
          ...
          : [{
              ...
              arrivalTime: `${8 + Math.floor(metrics.durationMinutes / 60)}:${String(metrics.durationMinutes % 60).padStart(2, '0')}`,
  ...
  try {
    ...
    showError(
      "Saved locally because server failed."
    );
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-4 gap-4">
                    ...
                    <div className="flex gap-1 bg-slate-50 p-1 rounded-xl border border-slate-100 overflow-x-auto no-scrollbar">
                      {['All', 'Beach', 'Hill Station', 'City'].map((cat) => (
                        <button
                          key={cat}
                          onClick={() => setSelectedCategory(cat as DestinationCategory | 'All')}
                          className={cn(
